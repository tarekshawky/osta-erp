...
export default async function AdminWarrantyCertificatesPage({
  searchParams,
}: {
  ...
  const [teams, totalCertificatesCount, certificateDates] = await Promise.all([
    prisma.team.findMany({ orderBy: { name: "asc" } }),
    prisma.warrantyCertificate.count(),
    prisma.warrantyCertificate.findMany({ select: { date: true } }),
  ]);

  const years = Array.from(new Set(certificateDates.map((c) => c.date.getFullYear()))).sort((a, b) => b - a);
  ...
  const filteredCount = await prisma.warrantyCertificate.count({ where });
    ...
    <div className="pb-10">
      ...
      <AdminTopBar
        ...
        dateFilter={{
          ...
          basePath: "/admin/warranty-certificates",
        }}
      />
      ...
      <div className="px-6 py-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h2 className="text-2xl font-bold text-slate-900">Warranty Certificates</h2>
            <p className="text-sm text-slate-500 mt-0.5">{totalCertificatesCount} records</p>
          </div>
          <Link
            href="/admin/warranty-certificates/new"
        ...
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-1 gap-3 max-w-xs">
          <AdminStatCard label="Total Certificates" value={String(totalCertificatesCount)} valueClassName="text-blue-700" />
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-slate-500 border-b border-slate-100">
                ...
                <th className="px-4 py-3 font-medium">Warranty Period</th>
              ...
              {certificates.map((c) => (
                <tr key={c.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                  <td className="px-4 py-3 font-medium text-blue-600 whitespace-nowrap">
                    <Link href={`/admin/warranty-certificates/${c.id}`}>{c.customerName}</Link>
                  </td>
                  <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{formatDate(c.date)}</td>
                  <td className="px-4 py-3 text-slate-900">{c.serviceProvided}</td>
                  <td className="px-4 py-3">
                    <TeamBadge name={c.team?.name} />
                  </td>
                  <td className="px-4 py-3 text-slate-600 whitespace-nowrap">
                    {c.createdBy.name.split(" ").slice(0, 2).join(" ")}
                  </td>
                  <td className="px-4 py-3 text-slate-600 whitespace-nowrap">
                    {formatDate(c.warrantyFrom)} - {formatDate(c.warrantyTo)}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <Link
                        href={`/admin/warranty-certificates/${c.id}/edit`}
                        title="Edit"
