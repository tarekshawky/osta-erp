...
import { getLabourSalesReport } from "@/lib/sparePartsReports";

function toDateInputValue(date: Date): string {
  return date.toISOString().slice(0, 10);
}

export default async function LabourSalesReportPage({
  searchParams,
}: {
  ...
  const report = await getLabourSalesReport(range);

  return (
    <div className="pb-10">
      <AdminTopBar title="Labour Sales Report" />
      <div className="px-6 py-6">
        <h2 className="text-2xl font-bold text-slate-900">Labour Sales Report</h2>
        <p className="text-sm text-slate-500 mt-0.5">Every Labour-type invoice line — type, quantity, pricing, and revenue</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-slate-500 border-b border-slate-100">
                <th className="px-4 py-3 font-medium">Labour Type</th>
            ...
            <tbody>
              {report.rows.map((r) => (
                <tr key={r.invoiceItemId} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                  <td className="px-4 py-3 text-slate-900 font-medium whitespace-nowrap">{r.labourType}</td>
                  <td className="px-4 py-3 text-right text-slate-600">{r.quantity.toLocaleString()}</td>
                  <td className="px-4 py-3 text-right text-slate-600 whitespace-nowrap">{formatAed(r.defaultPrice)}</td>
              ...
              {report.rows.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-4 py-10 text-center text-slate-400">
                    No Labour sales in this period.
                  </td>
                </tr>
              )}
            </tbody>
