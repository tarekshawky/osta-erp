...
import { processRefund } from "@/app/admin/invoices/[id]/actions";
...
export function RefundModal({
  ...
  function submit(refundAmount: number) {
    ...
    startTransition(async () => {
      const res = await processRefund(invoiceId, refundAmount);
      if (res.ok) {
        close();
        router.refresh();
      } else {
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4" onClick={close}>
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-900">Process Refund</h3>
            ...
            {mode === "choose" ? (
              ...
              <>
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
