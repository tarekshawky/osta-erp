    ...
    <div className="mb-6">
      <div className="flex items-center justify-between" dir={dir}>
        <button onClick={onBack} className="flex items-center gap-2 text-slate-900" aria-label="Back">
          ...
          <span>
            <span className={`block font-bold text-slate-900 leading-tight ${font}`}>{title}</span>
            <span className={`block text-xs text-slate-400 leading-tight ${font}`}>{subtitle}</span>
          </span>
        </button>
