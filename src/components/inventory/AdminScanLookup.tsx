...
import { lookupItemByCode, type ScanLookupResult } from "@/app/admin/inventory/scan/actions";
    ...
    <div className="max-w-md">
      <div className="flex gap-2">
        ...
        <button
          ...
          className="rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-700 disabled:opacity-50"
        >
          Look Up
        </button>
        <button
      ...
      {result && (
        <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
          <div className="flex items-start justify-between">
            ...
            <Link href={`/admin/inventory/items/${result.id}`} className="text-sm text-blue-600 hover:text-blue-700">
              View →
            </Link>
          </div>
          <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
            <div>
              <div className="text-xs text-slate-400">Main Warehouse</div>
              <div className="font-semibold text-slate-900">{result.mainQty.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-xs text-slate-400">Branches</div>
              <div className="font-semibold text-slate-900">{result.branchQty.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-xs text-slate-400">With Employees</div>
              <div className="font-semibold text-slate-900">{result.employeeQty.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-xs text-slate-400">Total Stock</div>
              <div className="font-semibold text-slate-900">{result.totalStock.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-xs text-slate-400">Cost Price</div>
              <div className="font-semibold text-slate-900">{result.costPrice != null ? formatAed(result.costPrice) : "—"}</div>
            </div>
            <div>
