...
import { transferToBranch } from "@/app/admin/inventory/actions";
...
export function WarehouseTransferModal({
  ...
  const [branchWarehouseId, setBranchWarehouseId] = useState(branches[0]?.id ?? "");
  ...
  function confirm() {
    ...
    startTransition(async () => {
      const res = await transferToBranch(mainWarehouseId, branchWarehouseId, inventoryItemId, Number(quantity), notes);
      if (res.ok) {
        close();
        router.refresh();
        showToast("Stock transferred to branch.");
      } else {
  ...
  return (
    <>
      <button
        ...
        disabled={items.length === 0 || branches.length === 0}
        className="text-sm font-medium bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-lg px-4 py-2.5"
      >
        + Warehouse Transfer
      </button>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4" onClick={close}>
          <div className="w-full max-w-sm rounded-xl bg-white p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-900">Warehouse Transfer</h3>
            ...
            <p className="mt-1 text-xs text-slate-500">Moves stock from the Main Warehouse into a Branch Warehouse.</p>

            <label className="mt-3 block text-xs font-medium text-slate-600">To Branch</label>
            <select
            ...
            <div className="mt-4 flex gap-3">
              ...
              <button
                type="button"
                disabled={isPending || !quantity || !inventoryItemId || !branchWarehouseId}
                onClick={confirm}
