...
import type { AssetFormInput } from "@/app/admin/financial-reports/assets/actions";

export type AssetFormValue = AssetFormInput;
...
export function AssetForm({
  ...
  onSave: (value: AssetFormInput) => Promise<{ ok: boolean; error?: string }>;
  onCancel: () => void;
}) {
  const [value, setValue] = useState<AssetFormValue>(initial);
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 mb-5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-xs font-medium text-slate-600">Purchase Cost (AED) *</span>
          <input
            type="number"
            min="0"
            step="0.01"
            ...
            onChange={(e) => setValue({ ...value, purchaseCost: Number(e.target.value) || 0 })}
          />
        </label>
        <label className="flex flex-col gap-1.5">
          ...
          <input
            ...
            onChange={(e) => setValue({ ...value, purchaseDate: e.target.value })}
          />
        </label>
        <label className="flex flex-col gap-1.5">
          <span className="text-xs font-medium text-slate-600">Useful Life (years) *</span>
          <input
            type="number"
            min="1"
            className="rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900"
            value={value.usefulLifeYears || ""}
            onChange={(e) => setValue({ ...value, usefulLifeYears: Number(e.target.value) || 0 })}
          />
          <span className="text-xs text-slate-400">Depreciated straight-line for the Balance Sheet&apos;s Net Book Value.</span>
        </label>
